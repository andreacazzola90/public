...
class CASE_Breadcrumb
{
    ...
    {
        $page_title = gurus()->get_theme_opt('page_title_search_result', 'Search');
        $page_on_front = get_option( 'page_on_front' );
        if($page_on_front) {
            $home_label = get_the_title( $page_on_front );
        } else {
            $home_label = esc_html__( 'Home', 'gurus' );
        }
        $args = wp_parse_args( $args, array(
            'home_label'            => $home_label,
            '404_label'             => esc_html__( 'Not Found', 'gurus' ),
            'search_results_label'  => esc_attr($page_title),
    ...
    function add_tag_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Tag:', 'gurus' ).' '.single_tag_title( '', false ) ).' '.get_tag_link( $queried_object->term_id
        );
    }
    ...
    function add_author_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Author:', 'gurus' ).' '.$userdata->display_name
        );
    }
    ...
    function add_tax_entry()
    {
         ...
         */
        $entries = apply_filters( 'gurus_breadcrumb_taxonomy', $entries, $this_term );
    ...
    function add_paged_entry()
    {
        ...
        {
            $this->add_entry(
                apply_filters( 'gurus_breadcrumb_paged', sprintf( 'Page %s', $page ) ),
                ''
            );
        }
    }
    ...
    function add_single_ancestor_entry( $post )
    {
         ...
         */
        $entries = apply_filters( 'gurus_breadcrumb_single', $entries, $post );
