...
$pxl_animate_item_first = $widget->get_setting('pxl_animate_item_first');
$pxl_animate_delay_item_first = $widget->get_setting('pxl_animate_delay_item_first');
...
$load_more = array(
    ...
    'html_id'         => $html_id,
    'pxl_animate_item_first' => $pxl_animate_item_first,
    'pxl_animate_delay_item_first' => $pxl_animate_delay_item_first,
);
$wrap_attrs = [
    'id'               => $html_id,
    'class'            => trim('pxl-grid pxl-service-grid pxl-service-grid-layout5'),
...
if( count($posts) <= 0){
    echo '<div class="pxl-no-post-grid pxl-dark-slate h2">'.esc_html__( 'No Post Found', 'gurus' ). '</div>';
    return;
} ?>
...
<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?>>
    ...
    <?php if ($pagination_type == 'pagination') { ?>
        <div class="pxl-grid-pagination">
            <?php gurus()->page->get_pagination($query, true); ?>
        </div>
    <?php } ?>
    <?php if (!empty($next_link) && $pagination_type == 'loadmore') { ?>
        <div class="pxl-load-more">
            <span class="btn-grid-loadmore btn-slip-primary">
                <span class="pxl-loadmore-text"><?php echo esc_html__('Load More', 'gurus') ?></span>
