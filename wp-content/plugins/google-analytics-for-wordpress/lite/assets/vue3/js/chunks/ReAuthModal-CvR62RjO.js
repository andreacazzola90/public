import { a5 as _export_sfc, o as openBlock, c as createElementBlock, a as createBaseVNode, t as toDisplayString, q as createCommentVNode, Q as renderSlot, F as Fragment, _ as __$1, L as defineComponent, a7 as h, aH as onBeforeMount, x as onMounted, K as getCurrentInstance, y as onBeforeUnmount, B as watch, j as ref, s as nextTick, aI as toRefs, k as getMiGlobal, D as createBlock, C as withCtx, T as Transition, u as unref, p as getUrl } from "./_plugin-vue_export-helper-mTbh_c-s.js";
...
const _sfc_main$2 = {
  ...
  props: {
    label: { type: String, default: __$1("Loading", "google-analytics-for-wordpress") },
  ...
  setup(__props) {
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
        ...
        __props.text ? (openBlock(), createElementBlock("span", _hoisted_2$2, toDisplayString(__props.text), 1)) : createCommentVNode("", true),
        renderSlot(_ctx.$slots, "default", {}, void 0, true)
      ], 64);
    };
  }
};
const LoadingSpinnerInline = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-f987e014"]]);
...
function arcToBezier(pos, val) {
  var rx = Math.abs(val[1]), ry = Math.abs(val[2]), xAxisRotation = val[3] % 360, largeArcFlag = val[4], sweepFlag = val[5], x = val[6], y = val[7], A2 = new Point(pos[0], pos[1]), B = new Point(x, y), primedCoord, lambda, mat, k, c, cSquare, t, O, OA, OB, tetaStart, tetaEnd, deltaTeta, nbSectors, f, arcSegPoints, angle, sinAngle, cosAngle, pt, i, il, retVal = [], x1, y1, x2, y2;
  ...
  OA = new Point(A2.x - O.x, A2.y - O.y);
...
function parseColor(str) {
  ...
  if (m2) return [+m2[1], +m2[2], +m2[3], m2[4] !== void 0 ? +m2[4] : 1];
  return null;
}
...
class Utils2 {
  ...
  getAllMarkers(filterCollapsed = false) {
    ...
    if (filterCollapsed) {
      markersWraps = markersWraps.filter((m2) => {
        const realIndex = Number(m2.getAttribute("data:realIndex"));
    ...
    let markers = [];
    markersWraps.forEach((m2) => {
      markers.push(m2.querySelector(".apexcharts-marker"));
    });
    return markers;
  }
...
class BoxCandleStick extends Bar {
  ...
  getOHLCValue(i, j) {
    ...
    const h2 = getCandleVal(w.globals.seriesCandleH);
    const o = getCandleVal(w.globals.seriesCandleO);
    const m2 = getCandleVal(w.globals.seriesCandleM);
    const c = getCandleVal(w.globals.seriesCandleC);
    const l = getCandleVal(w.globals.seriesCandleL);
    return {
      o: this.isBoxPlot ? h2 : o,
      h: this.isBoxPlot ? o : h2,
      m: m2,
...
function emitCoordinates(coords, row, rowLen, rowSum, xoffset, yoffset, width, height) {
  if (width >= height) {
    ...
    for (let i = 0; i < rowLen; i++) {
      const h2 = row[i] / areaWidth;
      coords.push([xoffset, subY, xoffset + areaWidth, subY + h2]);
      subY += h2;
    }
  } else {
...
const A = [
  ...
  setup(a, { emit: x }) {
    const g = ref(null), t = ref(null), f = (e) => e && typeof e == "object" && !Array.isArray(e) && e != null, S = (e, n) => {
      ...
      return t.value = new ApexCharts(g.value, s), t.value.render();
    }, d = () => (h2(), r()), h2 = () => {
...
function useChartColors() {
  const { colors, chartColors, name: name2 } = brandConfig;
...
const _hoisted_3 = { class: "monsterinsights-dark" };
const _hoisted_4 = { key: 0 };
const _hoisted_5 = ["disabled"];
const _hoisted_6 = { class: "monsterinsights-disclaimer-note" };
const _hoisted_7 = { key: 1 };
const _hoisted_8 = { class: "monsterinsights-dark" };
const _sfc_main$1 = {
  __name: "AuthModal",
  props: {
    isOpen: {
  ...
  emits: ["close"],
  setup(__props) {
    const { sprintf: sprintf2 } = wp.i18n;
    const isLoading = ref(false);
    const canUpdateSettings = getMiGlobal("can_view_reports", true);
    const productName = "MonsterInsights";
    const text_no_auth = __$1("You must connect with MonsterInsights before you can view reports.", "google-analytics-for-wordpress");
    const text_auth_label = __$1("MonsterInsights makes it effortless for you to connect your site with Google Analytics and see reports right here in the WordPress dashboard.", "google-analytics-for-wordpress");
    const text_wizard = __$1("Launch Setup Wizard", "google-analytics-for-wordpress");
    const text_ask_webmaster = __$1("Please ask your webmaster to connect MonsterInsights to Google Analytics.", "google-analytics-for-wordpress");
    const text_onboarding_note = sprintf2(__$1("Note: You will be transfered to %1$s.com to complete the setup wizard.", "google-analytics-for-wordpress"), productName);
    function launchSetupWizard() {
      ...
      wp.ajax.post(action, ajaxData).done((response) => {
        if (response && response.wizard_url) {
          ...
          window.location.href = getMiGlobal("wizard_url", "#");
        }
      }).fail(() => {
        window.location.href = getMiGlobal("wizard_url", "#");
      });
    ...
    return (_ctx, _cache) => {
      return openBlock(), createBlock(Transition, { name: "auth-fade" }, {
        default: withCtx(() => [
          __props.isOpen ? (openBlock(), createElementBlock("div", _hoisted_1$1, [
            createBaseVNode("h3", null, toDisplayString(unref(text_no_auth)), 1),
            createBaseVNode("div", _hoisted_2$1, [
              createBaseVNode("span", _hoisted_3, toDisplayString(unref(text_auth_label)), 1),
...
const AuthModal = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-4e28759b"]]);
const { __, sprintf } = wp.i18n;
const fetchAuth = () => new Promise((resolve, reject) => {
  const action = "monsterinsights_vue_get_profile";
  const ajaxData = {
    nonce: getMiGlobal("nonce")
  };
  wp.ajax.post(action, ajaxData).done((response) => {
    if (response && response.success === false) {
      reject({
        title: "Error fetching auth",
        message: response.message || "Server returned an error."
      });
    } else {
      resolve(response);
    }
    ...
    const message = errorThrown || __("An error occurred.", "google-analytics-for-wordpress");
    let title = __(
      "Error loading auth details",
...
const getAuthRedirect = (network = false) => {
  return new Promise((resolve, reject) => {
    const action = "monsterinsights_maybe_authenticate";
    const ajaxData = {
      nonce: getMiGlobal("nonce")
    };
    if (network) {
      ajaxData.isnetwork = true;
    }
    wp.ajax.post(action, ajaxData).done((response) => {
      resolve(response);
    }).fail((jqXHR, _textStatus, errorThrown) => {
      ...
      const message = errorThrown || __("An error occurred.", "google-analytics-for-wordpress");
      let title = __(
        "Error getting auth redirect",
        "google-analytics-for-wordpress"
      );
      if (jqXHR?.status) {
        title = sprintf(
          __(
            "Oops! We can't authenticate. Error: %1$s, %2$s.",
            "google-analytics-for-wordpress"
          ),
          jqXHR.status,
          jqXHR.statusText || errorThrown
...
const getReAuthRedirect = (network = false) => {
  return new Promise((resolve, reject) => {
    const action = "monsterinsights_maybe_reauthenticate";
    const ajaxData = {
      nonce: getMiGlobal("nonce")
    };
    if (network) {
    ...
    wp.ajax.post(action, ajaxData).done((response) => {
      ...
      const message = errorThrown || __("An error occurred.", "google-analytics-for-wordpress");
      let title = __(
        "Error getting re-auth redirect",
        "google-analytics-for-wordpress"
      );
      if (jqXHR?.status) {
        title = sprintf(
          __(
            "Oops! We can't reauthenticate. Error: %1$s, %2$s.",
            "google-analytics-for-wordpress"
          ),
          jqXHR.status,
          jqXHR.statusText || errorThrown
...
const verifyAuth = (network = false) => {
  return new Promise((resolve, reject) => {
    const action = "monsterinsights_maybe_verify";
    const ajaxData = {
      nonce: getMiGlobal("nonce"),
      isnetwork: network
      // In original it was formData.append('isnetwork', network);
    };
    wp.ajax.post(action, ajaxData).done((response) => {
      resolve(response);
    }).fail((jqXHR, _textStatus, errorThrown) => {
      ...
      const message = errorThrown || __("An error occurred.", "google-analytics-for-wordpress");
      let title = __("Error verifying auth", "google-analytics-for-wordpress");
      if (jqXHR?.status) {
        title = sprintf(
          __(
            "Oops! We can't verify credentials. Error: %1$s, %2$s.",
            "google-analytics-for-wordpress"
          ),
          jqXHR.status,
          jqXHR.statusText || errorThrown
...
const deleteAuth = (network = false, force = false) => {
  return new Promise((resolve, reject) => {
    const action = "monsterinsights_maybe_delete";
    const ajaxData = {
      nonce: getMiGlobal("nonce"),
      isnetwork: network
    };
    if (force) {
      ajaxData.forcedelete = true;
    }
    wp.ajax.post(action, ajaxData).done((response) => {
      resolve(response);
    }).fail((jqXHR, _textStatus, errorThrown) => {
      ...
      const message = errorThrown || __("An error occurred.", "google-analytics-for-wordpress");
      let title = __(
        "Error de-authenticating",
        "google-analytics-for-wordpress"
      );
      if (jqXHR?.status) {
        title = sprintf(
          __(
            "Oops! We can't verify deauthentication. Error: %1$s, %2$s.",
            "google-analytics-for-wordpress"
          ),
          jqXHR.status,
          jqXHR.statusText || errorThrown
...
const updateManualV4 = (v4 = "", network = false) => {
  return new Promise((resolve, reject) => {
    const action = "monsterinsights_update_manual_v4";
    const ajaxData = {
      manual_v4_code: v4,
      nonce: getMiGlobal("nonce")
    };
    if (network) {
      ajaxData.isnetwork = network;
    }
    wp.ajax.post(action, ajaxData).done((response) => {
      resolve(response);
    }).fail((jqXHR, _textStatus, errorThrown) => {
...
const updateMeasurementProtocolSecret = (value = "", network = false) => {
  return new Promise((resolve, reject) => {
    const action = "monsterinsights_update_measurement_protocol_secret";
    const ajaxData = {
      value,
      nonce: getMiGlobal("nonce")
    };
    ...
    wp.ajax.post(action, ajaxData).done((response) => {
      ...
      const message = errorThrown || __("An error occurred.", "google-analytics-for-wordpress");
      let title = __(
        "Error updating Measurement Protocol Secret",
        "google-analytics-for-wordpress"
      );
      if (jqXHR?.status) {
        title = sprintf(
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const text_no_auth = __$1("MonsterInsights encountered an error loading your report data", "google-analytics-for-wordpress");
    const text_auth_label = __$1("There is an issue with your Google Account authentication. Please use the button below to fix it by re-authenticating.", "google-analytics-for-wordpress");
    const text_button_reconnect = __$1("Reconnect MonsterInsights", "google-analytics-for-wordpress");
...
const ReAuthModal = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-4400042e"]]);
export {
  AuthModal as A,
